...
# Custom Claude Code statusline - Created: 2026-01-11T19:30:34.698Z
...
if [ -n "$session_id" ]; then
  STATE_FILE="$HOME/.config/claude-notify/state/${session_id}"
  if [ -f "$STATE_FILE" ]; then
    ...
    case "$STATE" in
      ...
      "permission")
        ...
        state_indicator="\033[43;30;1m          ⚠  PERMISSION          \033[0m"
        ;;
    esac
  fi
fi
...
# Line 1: Short directory and git only, plus state indicator
printf '📁 %s%s%s' "$(dir_color)" "$dir_name" "$(rst)"
