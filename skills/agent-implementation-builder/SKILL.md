...
12. **Fix-or-escalate (autonomy rule)** — Every finding (review issue, verifier mismatch, I/O-trace divergence) runs the decision in `references/common/autonomy-and-escalation.md`: resolvable from the spec/discovery → fix autonomously (if the *spec* is wrong, correct the spec, never code-only); genuinely undecided → escalate, fire-once, log a Known-Risk. The better the spec, the less this fires.
